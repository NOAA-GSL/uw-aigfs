...
Alternatively, for more human-friendly monitoring, use the ecFlow GUI -- see [ecFlow GUI (`ecflow_ui`)](#ecflow-gui-ecflow_ui) below.

If you need to stop the suite and change its configuration, potentially after re-running the `setup` command from [Setting Up the Final Config](#setting-up-the-final-config) to recreate the run directory with new configuration, or after manually editing `suite.def`:
